...
var playerBnds = {};
socket.on('update',(sid, id, childs, loc)=>{
    
    //console.log(sid, id, childs, loc);
    if(sid!=socket.id){
        
        console.log('updating');
        var bnd;
        if(sid in playerBnds){
            bnd = get(playerBnds[sid]);
        }
        else{
            bnd = get(addBoundary());
            playerBnds[sid] = bnd.id;
        } 
        
        //bnd.style = style;
        bnd.style.position = 'absolute';
        bnd.style.left = loc[0];
        bnd.style.top = loc[1];
        bnd.style.width = loc[2];
        bnd.style.height = loc[3];
        bnd.children[1].src = childs[1];
        // 
    }
...
function movBound(e){
    ...
    socket.emit('bndMove', bndLoc, bndChilds, clickId, socket.id);
}
